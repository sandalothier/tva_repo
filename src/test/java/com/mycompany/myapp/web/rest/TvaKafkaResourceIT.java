...
class TvaKafkaResourceIT {
    ...
    void setup() {
        ...
        TvaKafkaResource kafkaResource = new TvaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/tva-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/tva-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
